# Ride-Sharing Platform â€” Execution System
### Master Plan, Learning Strategy, Risk Management & Operating Rhythm

This document turns the onboarding blueprint and dependency graph into a runnable execution system. It assumes the build order established earlier: **User â†’ Driver â†’ Trip â†’ Config â†’ Eureka â†’ Gateway â†’ Location/PostGIS â†’ Matching â†’ RabbitMQ/Notification â†’ Payment â†’ Docker â†’ AWS/Monitoring.**

---

# PART 1 â€” Project Master Plan
...
- Exit criteria: You can create an entity, save/retrieve it via JPA, and issue + validate a JWT â€” without referring to a tutorial mid-task
...
- Milestones:
  ...
  - Trip Service: trip lifecycle states (REQUESTED â†’ MATCHED â†’ IN_PROGRESS â†’ COMPLETED/CANCELLED), calls User/Driver via hardcoded URLs
...
- Exit criteria: A full manual flow â€” register user, register driver, create trip, manually transition trip states â€” works end-to-end via hardcoded URLs
...
- Exit criteria: You can restart any service on a different port and the others still find it correctly â€” without code changes
...
**Theme:** Geospatial data â€” your second major learning gap
...
**Theme:** Orchestration across multiple services â€” first real distributed logic
...
- Expected outputs: Trip Service â†’ Matching â†’ Location/Driver flow working end-to-end through the Gateway
...
**Theme:** Asynchronous, event-driven communication â€” your third major gap
...
# PART 2 â€” Learning Sprint Plan

Each sprint maps to the phase it unlocks. **Just-in-time** â€” don't start a sprint until the previous phase's exit criteria are met.

### Sprint A â€” PostgreSQL + JPA/Hibernate (before Phase 1)
...
- **Why it matters:** Every service from Phase 1 onward needs this â€” it's the most foundational gap
- **Effort:** 4â€“6 focused sessions (~1.5â€“2 hours each)

### Sprint B â€” JWT Authentication (before Phase 1)
- **Learn:** Token structure, signing/verification, stateless auth filter in Spring Security
- **Don't learn yet:** OAuth2, refresh token rotation, SSO â€” overkill for this project's scope
- **Why it matters:** User Service issues tokens; every other service needs to validate them
- **Effort:** 2â€“3 sessions

### Sprint C â€” Spring Cloud Config (before Phase 2)
- **Learn:** External config repos, `@RefreshScope`, profile-based config (dev/prod)
- **Don't learn yet:** Vault/encrypted secrets management â€” add later if needed
- **Why it matters:** Foundation for Eureka and everything after
- **Effort:** 1â€“2 sessions (conceptually simple, mostly setup)

### Sprint D â€” Eureka Service Discovery (before Phase 3)
...
### Sprint E â€” Spring Cloud Gateway (before Phase 4)
...
### Sprint F â€” PostGIS (before Phase 5)
...
- **Why it matters:** Core to Location/Matching â€” and the concept (spatial indexes) doesn't exist in your current knowledge
- **Effort:** 3â€“4 sessions

### Sprint G â€” Concurrency & Idempotency Patterns (before Phase 6)
- **Learn:** Optimistic locking (`@Version`), idempotency keys, basic distributed-lock concepts
- **Don't learn yet:** Distributed consensus algorithms (Raft/Paxos) â€” not needed at this scale
- **Why it matters:** Without this, Matching Service will have race-condition bugs that are hard to reproduce later
- **Effort:** 2â€“3 sessions

### Sprint H â€” RabbitMQ + Event-Driven Patterns (before Phase 7)
- **Learn:** Exchanges, queues, bindings, publisher confirms, consumer acknowledgment, dead-letter queues
- **Don't learn yet:** Kafka, stream processing, exactly-once semantics â€” RabbitMQ's at-least-once + your own idempotency is sufficient
- **Why it matters:** Core async backbone of the system
- **Effort:** 3â€“4 sessions

### Sprint I â€” Docker + Docker Compose (before Phase 9)
- **Learn:** Dockerfiles for Spring Boot (multi-stage builds), Compose networking, volumes for Postgres data
- **Don't learn yet:** Kubernetes, Helm â€” Compose is the right tool for this project's scale
- **Why it matters:** Final packaging step before deployment
- **Effort:** 3â€“4 sessions

### Sprint J â€” AWS Production Deployment + Monitoring (before Phase 10)
- **Learn:** ECS (or EC2 + Docker), ALB basics, RDS, CloudWatch logs/metrics, ECR
- **Don't learn yet:** Multi-region failover, auto-scaling tuning, cost optimization â€” note these as "future work" in your README
- **Why it matters:** Final production step; observability is what makes the system "real"
- **Effort:** 4â€“6 sessions

---

# PART 3 â€” Technology Introduction Schedule

| Technology | Introduce At | Why Not Earlier |
|---|---|---|
| **PostgreSQL + JPA** | Phase 0 (before any service) | It's the data layer for everything â€” no service can exist without it |
| **JWT** | Phase 0 / Phase 1 | User Service's core function is auth; needed from the first real endpoint |
| **Config Server** | Phase 2 | Needs at least 2â€“3 working services with *something* to centralize â€” introducing it on day 1 with nothing to configure makes it abstract and pointless |
| **Eureka** | Phase 3 | Needs Config Server already in place (Eureka itself benefits from centralized config) and needs multiple real services to discover â€” otherwise it's "magic" with no payoff |
| **Gateway** | Phase 4 | Needs Eureka so it can route by logical service name â€” a Gateway without discovery is just a reverse proxy with hardcoded URLs, missing the point |
...
| **RabbitMQ** | Phase 7 | Needs a *real event* to publish (trip matched/completed from Phases 1â€“6) â€” learning messaging with no concrete event to send makes the pattern feel arbitrary |
| **Docker** | Phase 9 | Needs all services functionally complete â€” containerizing unfinished/buggy services means debugging business logic and Docker networking simultaneously (the #1 beginner trap per the risk analysis) |
| **AWS** | Phase 10 | Needs a working Docker Compose setup that mirrors production topology â€” deploying to AWS before this means debugging cloud infra AND containerization AND app logic at once |
| **Monitoring** | Phase 10 (alongside AWS) | Only meaningful once there's a deployed, running system to observe â€” monitoring an empty local dev setup teaches you the tool but not the *judgment* of what to watch for |
...
# PART 4 â€” Architecture Review Checklist
...
- [ ] Am I adding complexity "for later" that isn't justified by a current, real requirement? (If yes â€” cut it. YAGNI applies even in a learning project.)

---

# PART 5 â€” Deployment Review Checklist

Use this **for every service, every deployment** â€” local Docker Compose or AWS.

## Pre-Deployment Checklist
- [ ] All tests (or at minimum, the manual end-to-end flow) pass locally without Docker first
- [ ] Service's config values (DB connection, JWT secret, broker URL) are externalized â€” nothing hardcoded for "this environment"
...
- [ ] Deploy in dependency order: Config â†’ Eureka â†’ [services] â†’ Gateway â†’ RabbitMQ-dependent services last
- [ ] Only deploy **one new service at a time** if multiple are changing â€” isolate variables
- [ ] Watch logs live during startup â€” don't deploy-and-walk-away
- [ ] Confirm registration in Eureka dashboard (if applicable) immediately after startup

## Post-Deployment Checklist
- [ ] Hit `/actuator/health` on the deployed instance â€” confirm `UP`
...
- [ ] Check logs for any startup warnings, even if the service "came up fine" â€” silent config issues often log a warning, not an error
- [ ] Update `Deployment.md` with what was deployed, when, and the verification result

---

# PART 6 â€” Risk Register

| # | Risk | Warning Signs | Root Cause | Prevention | Recovery |
|---|---|---|---|---|---|
| 1 | Scope creep â€” adding features beyond the blueprint | Backlog growing faster than completed items; "wouldn't it be cool if..." moments | No defined "done" per phase | Exit criteria per phase (Part 1) are hard gates â€” write new ideas into a "Future Work" backlog instead | Review backlog weekly; explicitly defer items, don't silently abandon them |
...
| 13 | Burnout from learning too many new things at once | Avoidance of the project, frustration, "I don't understand any of this" feeling | Violating just-in-time learning â€” stacking multiple Level 3+ topics simultaneously | Strict adherence to Learning Sprint Plan (Part 2) and Weekly Workflow (Part 7) | Pause new learning entirely for a week; consolidate by re-doing the last working phase from memory |
| 14 | Losing the "why" behind early decisions | Can't explain in an interview why you chose X | No `Decisions.md` discipline | Log every non-trivial choice the day it's made | Reconstruct from git commit messages/PRs if needed â€” but this is a sign to start logging now |
...
| 17 | Local environment â‰  Compose â‰  AWS topology drift | Things that work in Compose fail on AWS | Different env var names, missing services in AWS setup | Keep Compose as the "spec" for what AWS must replicate | Diff Compose service list against AWS deployed services; close gaps one at a time |
...
| 20 | Portfolio presentation as an afterthought | Finished system, but no coherent story for interviews | Documentation treated as optional throughout | Engineering Notebook (Part 8) maintained continuously, not written retroactively | Block dedicated time in Phase 11 to consolidate notes into a narrative â€” budget for this as a real phase, not a footnote |

---

# PART 7 â€” Weekly Engineering Workflow

A repeatable weekly rhythm â€” adjust days to your schedule, but keep the *structure*.

### Monday â€” Planning + Architecture Review
- Review last week's exit criteria â€” were they actually met?
...
### Tuesdayâ€“Thursday â€” Learning + Coding (paired)
- **Just-in-time learning block first** (30â€“45 min): only the specific sprint topic needed for *this week's* milestone â€” not general study
...
### Friday â€” Integration + Debugging
...
### Saturday â€” Deployment Day (when a milestone is deployment-ready)
- Run **Pre-Deployment â†’ Deployment â†’ Post-Deployment Checklists** (Part 5)
- Update `Deployment.md`

### Sunday â€” Documentation + Reflection (light, ~30â€“60 min)
- Update `Architecture.md` if anything changed structurally
- Log any decisions made this week in `Decisions.md`
- Review the **Risk Register** â€” any new risks surfaced? Any warning signs from existing risks observed?
...
# PART 8 â€” Engineering Notebook Structure

A `/docs` folder in your repo with these files, maintained continuously (per Part 7's rhythm) â€” not written retroactively.
...
- **Update frequency:** Whenever a structural change happens (new service, new dependency between services, new infra component) â€” same day

### `Learning.md`
- **Purpose:** Running log of what you've learned, in your own words, mapped to the Learning Sprint Plan
- **Contents:** One entry per sprint â€” what was learned, what's still unclear, links/resources that actually helped (vs. ones that didn't)
- **Update frequency:** End of every session that involves new learning

### `Deployment.md`
- **Purpose:** History of every deployment â€” local and AWS â€” with outcomes
...
### `Debugging.md`
- **Purpose:** Catalog of non-trivial bugs and how they were diagnosed â€” your personal pattern library
...
### `Decisions.md`
- **Purpose:** Architecture Decision Records (ADRs) â€” the "why" behind non-obvious choices
...
# PART 9 â€” Personal Skill Growth

### Junior â†’ Intermediate (foundational shifts)

- **From "one database" to "data ownership"** â€” understanding that each service owning its data isn't bureaucracy, it's what enables independent scaling and deployment
- **From synchronous-only thinking to async-aware design** â€” recognizing *when* a REST call is wrong and an event is right
- **From "it compiles and runs" to "it's idempotent and handles retries"** â€” payment and matching logic forces this shift concretely
- **From copy-pasted config to externalized, environment-aware configuration** â€” Config Server makes "it works in dev but not prod" a solvable category of problem, not a mystery
- **From "the code is the documentation" to maintaining living architecture docs** â€” the Engineering Notebook habit
- **From PostgreSQL as "a place to store rows" to PostgreSQL with spatial awareness (PostGIS)** â€” a genuinely uncommon skill at the intermediate level

### Intermediate â†’ Advanced (systems-thinking shifts)

- **From "my service works" to "my service degrades gracefully"** â€” designing and *testing* failure modes, not just happy paths
- **From debugging by reading code to debugging by layered isolation** â€” the Debugging Playbook becomes second nature; you check dashboards and health endpoints before opening an editor
- **From "I added Docker" to understanding container networking deeply enough to diagnose it under pressure**
- **From "deployed it" to "deployed it incrementally, verified at each step, with a rollback plan"** â€” the deployment discipline in Part 5 becomes instinct
- **From risk-blind to risk-aware project execution** â€” maintaining and *acting on* a living risk register is a senior/staff-level habit, not just a junior checklist exercise
- **From "I built a system" to "I can reason about the ripple effects of changing any part of it"** â€” this is the Section 10 success criterion from the original blueprint, and it's the single biggest junior-to-senior transformation

---

# PART 10 â€” Mission Control Dashboard

A Notion-style dashboard structure. Recreate this as a Notion page (or a `DASHBOARD.md` with tables, or a simple project board) â€” the structure matters more than the tool.
...
|---|---|---|
| 1â€“20 | Not Yet Relevant / Monitoring / Mitigated / Occurred | |

*(Update during Sunday reflection â€” most risks stay "Not Yet Relevant" until their phase arrives)*
...
## ðŸ§­ This Week's Plan (Monday snapshot)
